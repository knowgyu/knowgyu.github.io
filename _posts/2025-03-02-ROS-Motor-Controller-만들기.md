...
이번 글에서는 기존에 제작한 모터 드라이버를 활용해, ROS 환경에서 모터와 서보를 제어할 수 있는 **`robot_motor_controller`** 패키지를 만드는 과정을 정리합니다.
...
> 모터 드라이버 관련 내용은 별도 글로 정리한 뒤 링크를 추가할 예정입니다.
{: .prompt-info }
...
`src/motor_controller_node.cpp` 파일에서는 ROS 노드를 초기화하고, 모터 컨트롤러 객체를 생성한 뒤 `ros::spin()`을 호출해 콜백 함수가 계속 실행되도록 합니다.  

> ROS **노드(Node)**란 하나의 실행 단위로, 서로 독립적으로 동작하며 메시지를 주고받는 기본 단위입니다.  
> 비유하자면 하나의 독립적인 프로세스에 가깝습니다.
{: .prompt-tip }
...
여기서 핵심 수정 사항은 아래와 같습니다.

### 라이브러리와 실행 파일 분리

- **라이브러리 추가:**  
  소스 파일들(`DC_motor.cpp`, `PCA9685.cpp`, `Servo_motor.cpp`, `i2c.cpp`)을 하나의 라이브러리로 묶어 다른 소스 파일에서도 재사용할 수 있도록 합니다.
