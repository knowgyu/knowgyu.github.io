...
하지만 애커만 조향 차량은 제자리 회전이 불가능하므로, 후진과 같은 다른 대안이 필요합니다.  

이번 글에서는 [backoff_recovery](https://github.com/maker-ATOM/backoff_recovery/) 저장소를 활용해 후진 리커버리 동작을 추가하는 방법을 정리합니다.  
...
backoff_recovery 패키지의 클래스 초기화 시 recovery 파라미터가 올바르게 반영되도록 네임스페이스를 설정해야 합니다.  
아래와 같이 코드를 수정합니다.
...
> 이 부분을 수정하지 않으면 recovery 파라미터 파일(`recovery_params.yaml`)에서 후진 속도나 후진 거리를 조정해도 값이 반영되지 않습니다.
{: .prompt-tip }
...
> 위 파라미터들도 실제 주행 환경에 맞춰 추후 튜닝할 예정입니다.
{: .prompt-tip }
...
이번 글에서는 ROS1 네비게이션 스택에 기본으로 제공되는 recovery behavior에 후진 리커버리 동작을 추가하는 방법을 정리했습니다.  

이처럼 ROS1에서 기본 제공되지 않는 기능도 GitHub에서 구현체를 찾아 활용하거나, 직접 플러그인을 작성해 확장할 수 있습니다.
