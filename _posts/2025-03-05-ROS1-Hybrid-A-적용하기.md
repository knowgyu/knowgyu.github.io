...
하지만 대부분은 그리드 기반 경로 탐색에 초점을 맞추고 있어, 실제 차량의 동역학 제약을 충분히 반영하지 못합니다.  

Hybrid A\* 알고리즘을 활용하면 기존 A\*와 달리 차량의 회전 반경과 비홀로노믹 제약을 고려한 경로 생성이 가능해집니다.  
이번 글에서는 ROS1 환경에서 Hybrid A\* 플러그인을 적용해 글로벌 플래너로 사용하는 방법을 정리합니다.  

> ROS1 `move_base`의 기본 Global Planner는 [ROS 위키 문서](https://wiki.ros.org/global_planner?distro=noetic)를 참고하면 됩니다.
{: .prompt-tip }
...
ROS2의 Nav2에서는 Hybrid A*를 기본 제공하지만, ROS1에서는 별도 구현체나 외부 플러그인을 활용해야 합니다.  
이번 예제에서는 [hybrid_astar_planner](https://github.com/dengpw/hybrid_astar_planner) 저장소를 기반으로 플러그인을 설치하고 활용하는 방법을 다룹니다.
...
> OMPL 관련 문제가 발생하면 OMPL의 설치 경로(예: `/usr/lib` 또는 `/usr/local/lib`)를 확인하고 환경변수 `OMPL_DIR`를 올바르게 설정해야 합니다.  
...
이제 기본적인 기능은 모두 추가했으니, 다음 글에서는 실제 파라미터 튜닝 과정을 정리할 예정입니다.  
