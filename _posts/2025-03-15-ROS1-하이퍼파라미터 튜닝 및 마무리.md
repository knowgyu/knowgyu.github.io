...
author: knowgyu
description: "ROS1 네비게이션 스택 하이퍼파라미터 튜닝 및 프로젝트 마무리에서 개요를 중심으로 설정·구현 흐름을 기록한다."
kind: project-log
...
당연한 이야기지만, PID 제어든 딥러닝 학습이든 파라미터 튜닝에 따라 성능 차이가 크게 납니다.  
이번 글에서는 제가 파라미터 튜닝을 하며 얻은 기준과 시행착오를 간단히 정리합니다.  
추가로 실제 주행 영상을 함께 첨부했습니다.  

## 개요

## 파라미터 튜닝

이전 글들에서 기본적인 `yaml` 파일 구성은 모두 마친 상태입니다.  
이제는 로봇이 좁은 통로를 안정적으로 지나가고, 목적지에도 정확히 도달할 수 있도록 파라미터를 세밀하게 조정해야 합니다.  

저는 파라미터 튜닝을 무작정 시작하기보다 다음과 같은 방식으로 접근했습니다.

- **공식 문서 이해**: 가장 먼저 ROS 위키와 공식 문서를 꼼꼼히 읽으며 각 파라미터가 정확히 어떤 역할을 하는지 이해했습니다. 이해 없이 무작정 값을 바꾸면 시간만 낭비됩니다.

- **범위 기반 테스트**: 일종의 파라메트릭 서치처럼 최소값과 최대값의 범위를 먼저 정한 뒤 테스트했습니다. 기본값을 기준으로 차근차근 조정하고, `rqt_reconfigure`를 활용하면 훨씬 수월하게 반복 실험할 수 있습니다.  

> PID gain값 튜닝을 할 때도 느끼지만, 정말 파라미터 튜닝은 정답이 없고 항상 상황마다 다릅니다.  
> 그렇기에 본인만의 기준을 세우고, 무작정 시간을 쓰기보다 조금이라도 효율적인 방법으로 접근하는 것이 중요합니다.
{: .prompt-info}

### `rqt_reconfigure` 활용

ROS에는 실시간으로 파라미터를 조정할 수 있는 `rqt_reconfigure`라는 유용한 도구가 있습니다. 이를 사용하면 YAML 파일을 수정하고 노드를 재시작하지 않아도 바로 결과를 확인할 수 있습니다.
...
- `max_vel_y`와 `min_vel_y`를 0으로 설정: 제 로봇은 차동 구동 방식이라 옆으로 움직일 수 없기 때문입니다. 전방향(holonomic) 로봇이었다면 이 값을 별도로 설정하는 것이 맞습니다.  
...
> 위 DWA 로컬 플래너에서도 언급했듯 비홀로노믹 로봇이기 때문에 제자리 회전이 불가능합니다.  
> 하지만 ROS1은 기본적으로 클리어맵과 제자리 회전을 제공하므로, 이전 글에서 Backoff 동작을 추가했습니다.  
> 한 차례만 수행하기보다 두 차례까지 후진을 허용하는 편이 대부분의 상황에서 더 잘 빠져나오고 목적지까지 도달할 수 있었습니다.
{: .prompt-tip }
...
프로젝트 회고 글을 따로 작성할지는 아직 모르겠지만, 이번 프로젝트는 개인적으로 꽤 재미있게 진행했습니다.  
특히 2D LiDAR 센서만으로 자율주행을 구현하는 과정은 생각보다 훨씬 까다로웠고, 파라미터 튜닝도 많은 시간을 요구했습니다.  

프로젝트를 수행하며 가장 크게 느끼고 배운 점을 정리하면 아래와 같습니다.

1. **문서를 꼼꼼히 읽자**
   공식 문서만큼 확실한 자료는 없습니다. 생성형 AI가 아무리 발전해도, 직접 공식 문서를 읽는 편이 결국 가장 효과적이었습니다.  

2. **rqt_reconfigure 활용**: 실시간으로 파라미터를 조정하고 결과를 즉시 확인하는 건 정말 정말 편리합니다.

3. **하드웨어 특성 고려**: 이론상 가능한 값이 실제 로봇에서는 작동하지 않을 수 있습니다. 특히 속도 관련 설정은 실제 모터 성능을 반드시 고려해야 합니다.
