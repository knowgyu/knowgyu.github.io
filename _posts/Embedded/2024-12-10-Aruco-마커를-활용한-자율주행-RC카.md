...
author: knowgyu
description: "ArUco 마커를 활용한 자율주행 RC카의 핵심 개념과 ArUco 마커를 활용한 자율주행 RC카 내용을 정리한다."
kind: technical-note
date: 2024-11-27 14:00:55 +0900
math: true
categories: [Embedded System, Control Engineering]
project: aruco-rc-car
...
하지만 ArUco 마커를 활용하면 위 센서들보다 저렴한 RGB 카메라만으로도 현재 위치를 알아낼 수 있어, 이를 활용한 프로젝트를 진행했습니다.
...
라즈베리파이에서 ArUco 마커를 인식하고, 명령어를 Renesas 보드로 전달해 정상적으로 RC카를 제어할 수 있었습니다.

현재 코드는 ArUco 마커의 ID만 활용해 좌회전 또는 정지 명령을 생성하므로 비교적 단순한 기능만 적용되어 있습니다.
하지만 마커의 pose estimation까지 활용하면 더 다양한 제어 로직으로 확장할 수 있을 것이라 기대합니다.

### GitHub Repo
프로젝트에 활용한 코드는 아래 링크에서 참고할 수 있습니다.
